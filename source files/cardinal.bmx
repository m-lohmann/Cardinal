		...
		Else 'If x<=Len(code[y])
			...
			Else
				Select Mid(code[y],x,1)
					...
					Case "(" rebuffer(x-1,y,Chr(val[on]));val[on]=grab(x+1,y);If remafter list.remove Self  ' fix: (x-1,x) => (x-1,y)
					Case ")" rebuffer(x+1,y,Chr(val[on]));val[on]=grab(x-1,y);If remafter list.remove Self  ' fix: (x+1,x) => (x+1,y)
					...
					Case "A" If dir=0 list.remove Self  'added by Manuel Lohmann, missing in the original implementation
					Case "V" If dir=1 list.remove Self  'added by Manuel Lohmann, missing in the original implementation
